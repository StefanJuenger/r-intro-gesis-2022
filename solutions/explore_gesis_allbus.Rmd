...
allbus_2021_subset <- read_sav("../data/allbus_2021ZA5280_v1-0-0.sav") %>% 
remove_all_labels()
...
allbus_2021_subset  <- allbus_2021_subset  %>% 
  ...
  mutate(trust = ifelse(trust == 1, 1, 0), #dichotomization
         ...
         party_vote = recode_factor(party_vote,
                                      ...
                                      `90` = "Other party",
                                      `91` = "Would not vote"),
         distrust_people = inverter(trust)) %>% 
  rowwise() %>% 
  mutate( across(contact1:contact4,
                ~recode(
                  .x,
                  `2` = 0))) %>% 
  mutate(contact = rowSums(across(contact1:contact4)), 
         xenophobia = rowMeans(across(xeno1:xeno4), na.rm = T)) %>% 
  ungroup()
```
...
model <- glm(trust ~ 1 + life_satf + contact + xenophobia,
