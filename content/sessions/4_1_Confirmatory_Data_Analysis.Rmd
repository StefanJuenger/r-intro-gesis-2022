...
```{r load-ggss-data}
ggss_2021 <- haven::read_spss("./data/ZA5280_v1-0-0.sav")
```
...
ggss_2021 <-
  ggss_2021 %>% 
  mutate(
    xenophobia = rowSums(across(ma01b:ma04)) - 4, 
    anti_immigrant = rowSums(across(mi05:mi11)) - 7,
    trust = ifelse(st01 == 1, 1, 0),
    contact = rowSums(across(mc01:mc04)) * -1 + 8
  )
```
...
```{r t-test}
t.test(xenophobia ~ eastwest, data = ggss_2021)
```
...
```{r swt}
shapiro.test(ggss_2021$xenophobia)
```
...
```{r wilcox}
wilcox.test(xenophobia ~ eastwest, data = ggss_2021)
```
...
```{r anova}
anova <- aov(xenophobia ~ as.factor(agec), data = ggss_2021)
...
```{r levene-test, eval = FALSE}
library(rstatix)

ggss_2021 %>% 
  levene_test(ps03 ~ as.factor(agec))
```

---

## Alternatives for ANOVA #XYZ
...
simple_linear_model <- 
  lm(xenophobia ~ as.factor(agec) + sex + eastwest + trust + contact, data = ggss_2021)
...
If you want to change the refence category for a categorical predictor, you can use the `base R` function `relevel()` within the regression formula.

```{r ref-cat, eval=FALSE}
lm_relevel <-  lm(risk_self ~ relevel(age_cat_num, ref = "10") + sex + left_right,
                           data = corona_survey)
