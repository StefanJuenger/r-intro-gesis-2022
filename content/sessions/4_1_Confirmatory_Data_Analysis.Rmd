...
.pull-left[
Say we're interested in xenophobic and anti-immigration attitudes. We'd like to investigate how social trust and contact to foreigners might reduce them.
...
allbus_2021_cda <-
  allbus_2021_cda %>% 
  mutate(
    xenophobia = rowSums(across(ma01b:ma04)) - 4, # sum score
    anti_immigrant = rowSums(across(mi05:mi11)) - 7, # sum score
    trust = ifelse(st01 == 1, 1, 0), # dichotomization
    contact = rowSums(across(mc01:mc04)) * -1 + 8, # sum score + inversion
    party = as.factor(pv01) # factor conversion
  )
```
...
class: center, middle
.small[
```{r ref.label = "report-regression", echo = FALSE, eval = FALSE}
```
]
