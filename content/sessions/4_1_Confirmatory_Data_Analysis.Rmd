...
allbus_2021_cda <-
  allbus_2021_cda %>% 
  mutate(
    # sum score + numeric conversion
    xenophobia = rowSums(across(ma01b:ma04, ~as.numeric(.x))) - 4, 
    # dichotomization
    trust = ifelse(st01 == 1, 1, 0), 
    # sum score + numeric conversion + inversion
    contact = rowSums(across(mc01:mc04, ~as.numeric(.x))) * -1 + 8, 
    party = pv01
  )
```
...
```{r t-test}
t.test(xenophobia ~ eastwest, data = allbus_2021_cda)
```
...
```{r wilcox}
wilcox.test(xenophobia ~ eastwest, data = allbus_2021_cda)
```
...
# [Exercise](https://stefanjuenger.github.io/r-intro-gesis-2022/exercises/Exercise_4_1_1_t-test_ANOVA.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`

## [Solutions](https://stefanjuenger.github.io/r-intro-gesis-2022/solutions/Exercise_4_1_1_t-test_ANOVA.html)
...
simple_linear_model <- 
  lm(
    xenophobia ~ age + sex + eastwest + trust + contact + party, 
...
lm_relevel <-  
  lm(
    xenophobia ~ age + sex + eastwest + trust + contact + relevel(party, ref = 6),
    data = allbus_2021_cda
  )
...
```{r effect-coding}
contrasts(allbus_2021_cda$party) <- contr.sum(8)
```
...
We can easily switch back to the default dummy coding using the `contr.treatment()` function.

.pull-left[
```{r dummy-coding}
contrasts(allbus_2021_cda$party) <- 
  contr.treatment(8, base = 1)
...
Of course, you also can use, e.g., survey design weights to adjust to estimates in `R`. For the `lm()` function, you would use the `weights` option.

```{r weight}
weighted_regression <-
  lm(
    xenophobia ~ age + sex + trust + contact + party,
    weights = wghtpew,
    data = allbus_2021_cda
  )
...
simple_linear_model_logistic <- 
  glm(
    trust ~ age + sex + eastwest + contact + party, 
...
```{r compare-perf}
compare_performance(
  simple_linear_model_logistic, simple_linear_model_probit,
...
# [Exercise](https://stefanjuenger.github.io/r-intro-gesis-2022/exercises/Exercise_4_1_2_Regression_Analysis.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`

## [Solutions](https://stefanjuenger.github.io/r-intro-gesis-2022/solutions/Exercise_4_1_2_Regression_Analysis.html)
...
It is also quite straightforward to produce simple predictions from an estimated model using the `predict()` function in `R`. We can feed the `predict()` function with our own data to figure out what our model actually predicts when something changes.
...
simple_linear_model_new <-  
  lm(xenophobia ~ age + eastwest + party, data = allbus_2021_cda)

predictions_data <-  
  data.frame(
    age = rep(mean(allbus_2021_cda$age, na.rm = TRUE), times = 4),
    eastwest = as.factor(rep(1:2, each = 2)),
    party = as.factor(rep(c(1, 42), times = 2))
  )
...
```{r pred-log}
simple_linear_model_logistic_new <- 
  glm(
    trust ~ age + eastwest + party,
    family = binomial(link = "logit"),
    data = allbus_2021_cda
  )

predictions <- 
  predict(object = simple_linear_model_logistic_new, newdata = predictions_data)
```
...
stargazer(
  lm(xenophobia ~ age + sex + eastwest, data = allbus_2021_cda),
  lm(xenophobia ~ age + sex + eastwest + trust, data = allbus_2021_cda),
  glm(trust ~ age + sex + eastwest, data = allbus_2021_cda, family = binomial(link = "logit")),
  glm(trust ~ age + sex + eastwest, data = allbus_2021_cda, family = binomial(link = "probit")),
  type = "text",
  dep.var.labels = c("Xenophobia", "Trust"),
  covariate.labels =
    c("Age", "Sex", "Region (Reference = Western Germany", "Social Trust")
)
```
...
.small[
*Note*: While `report` is format-agnostic as it produces plain-text output, a nice tool for getting your output from `R` into *Microsoft Word* is the [`tidystats` Word add-in](https://www.tidystats.io/word-add-in.html).
]
...
# [Exercise](https://stefanjuenger.github.io/r-intro-gesis-2022/exercises/Exercise_4_1_3_Regression_Reporting.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`

## [Solutions](https://stefanjuenger.github.io/r-intro-gesis-2022/solutions/Exercise_4_1_3_Regression_Reporting.html)
