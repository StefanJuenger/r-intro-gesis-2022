...
The first step is loading the data into `R`. The *Public Use File (PUF) of the GESIS German General Social Survey (ALLBUS) 2021* is available in different formats. We will work with the `.csv` file.

```{r load-albus2021-data}
allbus2021<- read_csv2("./data/ZA5667_v1-1-0.csv")
```
...
We might want to reduce our data frame (or create a new one) to only include a *subset of specific variables*. Say, for example, we want to select only the variables that measure the satisfaction with democracy, life satisfaction and information seeking on social media from our full data set. There are two options for doing this with `base R`:

Option 1
.small[
```{r select-vars-base}
allbus_democracy_satisf <- allbus2021[, c("ps03", "ls01", "lm35")]
...
.small[
```{r subset}
allbus_democracy_satisf  <- subset(allbus2021, TRUE, select = c(ps03, ls01, lm35))
...
```{r select}
allbus_democracy_satisf <- allbus2021 %>% 
  select(ps03,
         ls01,
         lm35)

head(allbus_democracy_satisf)
```
...
.small[
```{r unselect-base}
allbus2021_cut <- allbus2021[!(names(allbus2021) %in% c("age", "sex"))]
...
.small[
```{r subset-unselect}
allbus2021_cut  <- subset(allbus2021, TRUE, select = -c(age, sex))
...
```{r select-helper}
allbus2021_pt <- allbus2021 %>% 
  select(starts_with("pt"))

allbus2021_1 <-allbus2021 %>% 
  select(ends_with("_1"))

glimpse(allbus2021_1)
```
...
```{r rename}
allbus_democracy_satisf  <- allbus_democracy_satisf %>% 
