...
hist(allbus_2021$sum_trust, breaks =15)
```
]
...
plot(
  jitter(allbus_2021$agec, 2), 
  jitter(allbus_2021$sum_trust, 2),
  pch = 16
)
```
]
...
(my_barplot | 
   ~barplot(table(allbus_2021$agec))
) /
  (my_boxplot | 
     ~boxplot(allbus_2021$agec)
  )
```
]
...
#allbus_2021$pv01<-haven::as_factor(allbus_2021$pv01)
...
## Barplots in descriptive statistics

```{r barplot-descriptives, eval = FALSE}
library(summarytools)

allbus_2021 %>% 
  select(educ, sum_trust) %>% 
  dfSummary( 
    plain.ascii  = FALSE,
    style        = 'grid',
    varnumbers = FALSE,
...
.tinyish[
```{r barplot-descriptives-print, echo = FALSE}
library(summarytools)
