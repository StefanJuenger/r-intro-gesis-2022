...
table(allbus2021$hzcy052a, allbus2021$hzcy052aR)
```
]
...
.small[
```{r set-NA-var-base}
sum(is.na(allbus2021$hzcy006a))

allbus2021$hzcy006a[allbus2021$hzcy006a == -99] <- NA
allbus2021$hzcy006a[allbus2021$hzcy006a == -77] <- NA
allbus2021$hzcy006a[allbus2021$hzcy006a == -33] <- NA
...
Another easy-to-use option for recoding values to `NA` (for individual variables of full data frames) is the function `set_na()` from the `sjlabelled` package. We can, e.g., use it the same way we have used the `naniar` function `replace_with_na_all()` in the previous example.
