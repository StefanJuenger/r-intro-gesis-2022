...
It is good practice to name code chunks. In the above example `{r cars}` specifies the language for the code `r` and a name `cars`. By naming code chunks it is possible to reference them in other code chunks and they will also appear in the interactive ToC at the bottom of the tab for the `R Markdown` document. Chunk names may never be used twice in a single document and should not include spaces or underscores.
...
- [`redoc`](https://github.com/noamross/redoc) "is a package to enable a two-way R Markdown-Microsoft Word workflow" (**NB**: The package is currently "in suspended animation" and not available via CRAN) 
...
# [Exercise](https://stefanjuenger.github.io/r-intro-gesis-2022/exercises/Exercise_5_1_1_R_Markdown.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`
